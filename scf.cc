...
Matrix compute_2body_fock(const std::vector<libint2::Shell>& shells,
                                 const Matrix& D);
Matrix compute_1_body_ints_MO(Matrix& onebodyintsSO, Matrix& fock);
...
Matrix compute_1_body_ints_MO(Matrix& onebodyintsSO, Matrix& C)
{
    Matrix onebodyintsMO = Matrix::Zero(nao,nao);
    for(auto p=0; p<nao; ++p) {
        for(auto q=0; q<nao; ++q){
            for(auto mu=0; mu<nao; ++mu){
                for(auto v=0; v<nao; ++v){
                    onebodyintsMO(p,q) += C(p,mu)*onebodyintsSO*C(v,q);
                }
            }
        }
    }
    Matrix onebodyintsMOjustmatMult = C.transpose()*onebodyintsSO*C;
   Print("some bullshit easy way            some harder way");
   Print(onebodyintsMOjustmatMult, "         ", onebodyintsMO);
